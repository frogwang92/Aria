...
class DATA_CLASS DataSlice {
 ...
 public:
  ...
  bool add_data(Symbol symbol, const shared_ptr<OHLCV> &p_data);
  bool add_indicator(Symbol symbol, const std::string &indicator_name, double value);

  shared_ptr<OHLCV> get_data(const Symbol &symbol);

  shared_ptr<OHLCV> operator[](const Symbol &symbol) { return get_data(symbol); }

 private:
  Resolution m_res;
  boost::posix_time::ptime m_starttime;
  boost::posix_time::ptime m_endtime;
  std::map<Symbol, shared_ptr<OHLCV> > m_data;
